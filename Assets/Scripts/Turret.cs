...
public class Turret : MonoBehaviour
{
    ...
    public List<EnemyAi> target = new List<EnemyAi>();
    ...
    void Update()
    {
        ...
        if (currentEnemyTarget != null)
        {
            ...
            {
                target.Remove(currentEnemyTarget);
            }
            RotateTowardsTarget();
            if (shootingTimer <= 0f)
            {
    ...
    private void RotateTowardsTarget()
    {
        ...
        player.LookAt(currentEnemyTarget.transform.position);
    ...
    void Shoot()
    {
        ...
        DestroyTurretBul dam =  bullet.GetComponent<DestroyTurretBul>();
        dam.damage = turretDamage;
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Enemy"))
        {
            ...
            if (target.Contains(oldEnemy))
            {
                target.Remove(oldEnemy);
            }
        }
    }
